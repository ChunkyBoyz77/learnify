    ...
    <div class="py-10 max-w-5xl mx-auto px-4">
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-teal-200">
            ...
            <form action="{{ route('lessons.update', $lesson->id) }}" 
                ...
                {{-- VIDEO SECTION --}}
                <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">Video</h3>
                ...
                @if($video)
                    <p class="mb-3 text-gray-600 dark:text-gray-300">
                        Current Video: <a class="text-teal-600 underline" href="{{ asset('storage/'.$video->file_path) }}">View</a>
                    </p>
                @elseif($videoUrl)
                    <p class="mb-3 text-gray-600 dark:text-gray-300">
                        Current Video URL: <a class="text-teal-600 underline" href="{{ $videoUrl->file_path }}" target="_blank">Open</a>
                    </p>
                @else
                    <p class="mb-3 text-gray-600 dark:text-gray-300">No video uploaded yet.</p>
                @endif
                ...
                {{-- NOTES SECTION --}}
                <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">Notes File (PDF)</h3>
                ...
                @if($pdf)
                    <p class="mb-3 text-gray-600">
                        Current Notes: <a class="text-teal-600 underline" 
                        href="{{ asset('storage/'.$pdf->file_path) }}" target="_blank">Download</a>
                    </p>
                @else
                    <p class="mb-3 text-gray-600">No notes uploaded yet.</p>
                @endif

                <input type="file" name="note_file" accept="application/pdf" class="block w-full mb-8 border rounded-lg">
                ...
                <div class="mt-6">
                    ...
                    @if($quiz)
                        <a href="{{ route('courses.quiz.editor', $quiz->id) }}"
